...
def show_choices(name1, name2, op):
    """Filter choices based on what was typed in the entry"""
    global alphabetical_rocket_choices
    pattern = entryVar.get()
    alphabetical_rocket_choices = [x for x in alphabetical_rocket_choices if x.startswith(pattern)]
    lb.delete(0, "end")
...
#*** Create Listbox & Populate listbox ***
...
show_choices_partial = partial(show_choices, rocketDictionary.keys())
entryVar.trace("w", show_choices)
